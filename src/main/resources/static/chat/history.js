export function createHistory({elements, fetchJson, localDateValue, calendarMonthsAgo,
  ...
  const state = {gid: group.gid || null, page: 1, total: 0, query: null, targetMid: null,
  ...
  function renderResults(items) {
    elements.historyResultsList.replaceChildren(...items.map(message => {
      const button = document.createElement("button"); button.className = "history-result";
      ...
      button.append(time, sender, summary); button.addEventListener("click", () => openContext(message));
      return button;
    }));
    ...
    elements.historyPrevious.disabled = state.page <= 1; elements.historyNext.disabled = state.page >= pageCount;
  ...
  function cursorRequest(direction, message) {
    ...
    query.set(`${direction}CreatedAt`, String(message.createdAt)); query.set(`${direction}Mid`, String(message.mid));
    return fetchJson(`/chat/messages/cursor?${query}`, {cache: "no-store"});
  }
  async function openContext(target) {
    const version = ++state.requestVersion; state.loadingMore = false; state.targetMid = target.mid;
    ...
    try {
      ...
      elements.historyMessages.replaceChildren(...messageElements([...before.items, target, ...after.items])); updateEdges();
      elements.historyMessages.querySelector(`[data-mid="${target.mid}"]`)?.scrollIntoView({block: "center"});
    } catch { if (version === state.requestVersion) elements.historyFeedback.textContent = "消息上下文加载失败，请返回后重试。"; }
  }
  async function loadMore(direction) {
    const earlier = direction === "before", cursor = earlier ? state.beforeCursor : state.afterCursor;
    if (!cursor || state.loadingMore) return;
    state.loadingMore = true; const version = state.requestVersion;
    const anchor = earlier ? captureScrollAnchor(elements.historyMessages) : null;
    try {
      const result = await cursorRequest(direction, cursor); if (version !== state.requestVersion) return;
      const loaded = messageElements(result.items); const next = result.hasMore ? {
        createdAt: earlier ? result.nextBeforeCreatedAt : result.nextAfterCreatedAt,
        mid: earlier ? result.nextBeforeMid : result.nextAfterMid} : null;
      if (earlier) { state.beforeCursor = next; elements.historyMessages.prepend(...loaded); restoreScrollAnchor(anchor, elements.historyMessages); }
      else { state.afterCursor = next; elements.historyMessages.append(...loaded); if (loaded[0]) scrollMessageToStart(loaded[0]); }
      updateEdges();
    } catch { if (version === state.requestVersion) (earlier ? elements.historyEarlierState : elements.historyNewerState).textContent = earlier ? "更早消息加载失败" : "更新消息加载失败"; }
    finally { if (version === state.requestVersion) state.loadingMore = false; }
  }
  async function query(page) {
    const version = ++state.requestVersion, query = new URLSearchParams({gid: String(state.gid), page: String(page), size: String(searchPageSize)});
    const filters = state.query; if (filters.start) query.set("start", `${filters.start} 00:00:00`);
    if (filters.end) query.set("end", `${filters.end} 23:59:59`); if (filters.sender) query.set("senderName", filters.sender); if (filters.keyword) query.set("keyword", filters.keyword);
    elements.historyEmpty.hidden = true; elements.historyContext.hidden = true;
    try { const result = await fetchJson(`/chat/messages?${query}`, {cache: "no-store"}); if (version !== state.requestVersion) return;
      state.page = result.page; state.total = result.total; renderResults(result.items); elements.historyFeedback.textContent = result.items.length ? "" : "没有符合条件的聊天记录";
    } catch { if (version === state.requestVersion) { elements.historyResults.hidden = true; elements.historyFeedback.textContent = "聊天记录查询失败，请稍后重试。"; } }
  }
  async function capture() {
    if (!state.gid) return; elements.historyEmpty.hidden = true; elements.historyResults.hidden = true; elements.historyContext.hidden = true;
    const raw = elements.historySyncTime.value; if (!raw) { elements.historyFeedback.textContent = "请先选择要同步到的历史日期。"; return; }
    const version = ++state.requestVersion;
    try { const query = new URLSearchParams({gid: String(state.gid), sinceTime: `${raw} 00:00:00`}); const response = await fetch(`/chat/since?${query}`, {method: "POST"}); if (!response.ok) throw new Error(); if (version === state.requestVersion) elements.historyFeedback.textContent = "已开始同步更早的历史消息，稍后请手动刷新查看。"; }
    catch { if (version === state.requestVersion) elements.historyFeedback.textContent = "同步历史请求失败，请稍后重试。"; }
  }
  function close() { state.requestVersion += 1; if (elements.historyDialog.open) elements.historyDialog.close(); }
  function open() { reset(); elements.historyDialog.showModal(); }
  function setGroup(nextGroup) { state.gid = nextGroup.gid; state.group = nextGroup; elements.historyOpen.disabled = false; elements.historyTitle.textContent = `聊天记录 - ${nextGroup.name || `群聊 ${nextGroup.gid}`}`; }
  elements.historyOpen.addEventListener("click", open); elements.historyClose.addEventListener("click", close);
  elements.historyForm.addEventListener("submit", event => { event.preventDefault(); state.query = {start: elements.historyStart.value, end: elements.historyEnd.value, sender: elements.historySender.value.trim(), keyword: elements.historyKeyword.value.trim()}; query(1); });
  elements.historyPrevious.addEventListener("click", () => query(state.page - 1)); elements.historyNext.addEventListener("click", () => query(state.page + 1)); elements.historySync.addEventListener("click", capture);
  elements.historyBack.addEventListener("click", () => { state.requestVersion += 1; elements.historyContext.hidden = true; elements.historyFeedback.textContent = ""; elements.historyResults.hidden = false; });
  elements.historyMessages.addEventListener("scroll", () => { if (elements.historyMessages.scrollHeight <= elements.historyMessages.clientHeight) return; if (elements.historyMessages.scrollTop <= earlierLoadThreshold) return loadMore("before"); const distance = elements.historyMessages.scrollHeight - elements.historyMessages.scrollTop - elements.historyMessages.clientHeight; if (distance <= earlierLoadThreshold) loadMore("after"); });
